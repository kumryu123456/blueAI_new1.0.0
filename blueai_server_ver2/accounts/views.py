...
def password_reset_request_view(request):
    ...
    if request.method == 'POST':
        form = PasswordResetForm(request.POST)
        if form.is_valid():
            email_or_phone = form.cleaned_data.get('email_or_phone', form.cleaned_data.get('email', ''))
            ...
            if is_email:
                ...
                # For demo: show verification code (in real world, this would be sent via SMS)
                messages.info(request, '테스트 용도: 인증코드는 123456입니다.')
