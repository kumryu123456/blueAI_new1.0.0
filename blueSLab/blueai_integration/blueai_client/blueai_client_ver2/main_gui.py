...
class BlueAIGUI(QMainWindow):
    ...
    def init_settings_tab(self):
        ...
        # 브라우저 선택
        auto_layout.addWidget(QLabel("브라우저:"), 2, 0)
        self.browser_combo = QComboBox()
        self.browser_combo.addItems(["Edge", "Chrome", "Firefox"])
        auto_layout.addWidget(self.browser_combo, 2, 1)
        
        # 인터럽션 처리 옵션
        self.handle_cookies = QCheckBox("쿠키 알림 자동 처리")
        self.handle_cookies.setChecked(True)
        auto_layout.addWidget(self.handle_cookies, 3, 0, 1, 2)
        
        self.handle_popups = QCheckBox("팝업 자동 처리")
        self.handle_popups.setChecked(True)
        auto_layout.addWidget(self.handle_popups, 4, 0, 1, 2)
        
        self.handle_ads = QCheckBox("광고 자동 처리")
        self.handle_ads.setChecked(True)
        auto_layout.addWidget(self.handle_ads, 5, 0, 1, 2)
        
        # 타임아웃 설정
        auto_layout.addWidget(QLabel("탐색 타임아웃 (초):"), 6, 0)
        self.navigation_timeout = QLineEdit("30")
        auto_layout.addWidget(self.navigation_timeout, 6, 1)
        
        auto_layout.addWidget(QLabel("요소 타임아웃 (초):"), 7, 0)
        self.element_timeout = QLineEdit("5")
        auto_layout.addWidget(self.element_timeout, 7, 1)
    ...
    def get_current_settings(self) -> Dict[str, Any]:
        ...
        browser_config = {
            ...
            'timeout': int(navigation_timeout * 1000),  # 밀리초로 변환
