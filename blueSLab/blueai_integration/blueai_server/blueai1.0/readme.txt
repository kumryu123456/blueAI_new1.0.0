...
Update 내용(파일별 분석):
accounts/backends.py (커스텀 인증 백엔드) -> 이메일이나 사용자를 통해, 사용자 연동
accounts/migrations/0001_initial.py (데이터베이스 초기 마이그레이션) -> 데이터 베이스 생성
accounts/context_processors.py (템플릿 컨텍스트 프로세서) -> 저장되어있는 데이터 베이스에서 템플릿(프론트엔드)에 반영 쉽게하기 위한 코드
accounts/forms.py -> 회원가입 시스템(아이디, 이메일 중복 및 비밀번호 같은지 검사)
accounts/models.py -> 